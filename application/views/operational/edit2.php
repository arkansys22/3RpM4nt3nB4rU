        ...
        <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
          <div class="grid grid-cols-12 gap-4 md:gap-6 2xl:gap-9">
            <div class="col-span-12 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
              ...
              <form action="<?= site_url('finance-operational/update2/'.$pc->id_session) ?>" method="post" class="bg-white p-6 shadow-md rounded">
              <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">
                ...
                <div id="staffGajiField" class="hidden">
                    ...
                    <select name="staff_id_session" class="w-full px-4 py-2 border rounded mb-4">
                        ...
                        <option value="<?= $c->id_session ?>" <?= (isset($pc->staff_id_session) && $pc->staff_id_session === $c->id_session) ? 'selected' : '' ?>><?= htmlspecialchars($c->nama) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <input type="hidden" name="url_back" value="<?= $url ?>" />
                ...
                <a href="<?=$url?>" class="sm:ml-2 bg-gray-500 text-white px-4 py-2 rounded w-full hover:bg-gray-600 sm:w-24 text-center">Batal</a>
                </div>
              </form>
            </div>
          </div>
