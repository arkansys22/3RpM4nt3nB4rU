        ...
        <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
          <div class="grid grid-cols-12 gap-4 md:gap-6 2xl:gap-9">
            <div class="col-span-12 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
              ...
              <form action="<?= base_url('Crud_payment/update/' . $project->id_session . '/' . $invoice_number) ?>" method="post" class="bg-white p-6 shadow-md rounded">
                <label class="block mb-2">Invoice</label>
                <input type="text" name="invoice_<?= $invoice_number ?>" value="<?= $payment->{"invoice_" . $invoice_number} ?>" class="w-full px-4 py-2 border rounded mb-4" required>

                <label class="block mb-2">Kwitansi</label>
                <input type="text" name="kwitansi_<?= $invoice_number ?>" value="<?= $payment->{"kwitansi_" . $invoice_number} ?>" class="w-full px-4 py-2 border rounded mb-4" required>

                <label class="block mb-2">Amount</label>
                <input type="text" id="formattedNumber" oninput="formatNumber(this)" name="amount_<?= $invoice_number ?>" step="0.01" value="<?= $payment->{"amount_" . $invoice_number} ?>" class="w-full px-4 py-2 border rounded mb-4" required>

                <label class="block mb-2">DP</label>
                <input type="text" id="formattedNumber" oninput="formatNumber(this)" name="dp_<?= $invoice_number ?>" step="0.01" value="<?= $payment->{"dp_" . $invoice_number} ?>" class="w-full px-4 py-2 border rounded mb-4" required>
                ...
                <input type="date" name="due_date_<?= $invoice_number ?>" value="<?= isset($payment) ? $payment->{"due_date_" . $invoice_number} : '' ?>" class="w-full px-4 py-2 border rounded mb-4">
...
<script>
// Menyimpan tanggal jatuh tempo ke sessionStorage (atau localStorage)
