...
<head>
    ...
    <title>Top Sales - <?= date('F Y', strtotime($periode . '-01')) ?></title>
        ...
        <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
          <div class="grid grid-cols-12 gap-4 md:gap-6 2xl:gap-9">
            <div class="col-span-12 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
                ...
                <h1 class="text-2xl font-bold">Top Sales - <?= date('F Y', strtotime($periode . '-01')) ?></h1>
                <!-- Tombol Kembali -->
                ...
                <div class="flex items-center justify-between mb-4 gap-3">
                  <a href="<?= site_url('sales-ranking/' . $periode_sebelumnya) ?>" class="bg-gray-200 dark:bg-neutral-700 text-gray-700 dark:text-gray-200 px-4 py-2 rounded-md hover:bg-gray-300 dark:hover:bg-neutral-600">
                  ...
                  <form method="get" onsubmit="return false;">
                    <input
                      ...
                      onchange="window.location.href = '<?= site_url('sales-ranking/') ?>' + this.value"
                    >
                  </form>

                  <a href="<?= site_url('sales-ranking/' . $periode_berikutnya) ?>" class="bg-gray-200 dark:bg-neutral-700 text-gray-700 dark:text-gray-200 px-4 py-2 rounded-md hover:bg-gray-300 dark:hover:bg-neutral-600">
                ...
                <div class="overflow-x-auto bg-white dark:bg-neutral-700 rounded-lg shadow-md">
                  <table class="min-w-full text-left text-sm whitespace-nowrap bg-white dark:bg-neutral-800">
                    <thead class="uppercase tracking-wider border-b-2 border-gray-200 dark:border-neutral-600 bg-gray-100 dark:bg-neutral-700">
                      <tr>
                        <th scope="col" class="px-6 py-4 text-gray-700 dark:text-gray-300">Peringkat</th>
                        <th scope="col" class="px-6 py-4 text-gray-700 dark:text-gray-300">Nama Sales</th>
                    ...
                    <tbody>
                      ...
                      <?php else: ?>
                        <?php foreach ($top_sales_ranking as $i => $rank): ?>
                          <tr class="border-b border-gray-200 dark:border-neutral-600">
                            <td class="px-6 py-4 text-gray-900 dark:text-gray-200">#<?= $i + 1 ?></td>
                            <td class="px-6 py-4 text-gray-700 dark:text-gray-400">
                              <a href="<?= site_url('sales-achievement/' . $rank->user_id_session) ?>" class="text-blue-500 hover:underline">
                                <?= $rank->nama ?>
                              </a>
                            </td>
                            <td class="px-6 py-4 text-gray-700 dark:text-gray-400">
                              Rp <?= number_format($rank->total_pencapaian, 0, ',', '.') ?>
                            </td>
                          </tr>
                        <?php endforeach; ?>
                      <?php endif; ?>
