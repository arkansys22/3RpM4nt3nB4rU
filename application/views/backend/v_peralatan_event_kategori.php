...
<head>
    ...
    <title>Kategori Peralatan Event</title>
          ...
          <div class="grid grid-cols-12 gap-4 md:gap-6 2xl:gap-9">
            <div class="col-span-12 lg:col-span-8 lg:col-start-3 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
              ...
              <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-4">
                <h1 class="text-2xl font-bold">Kategori Peralatan Event</h1>
                <a href="<?= site_url('peralatan-event') ?>"
              ...
              <p class="text-sm text-body dark:text-bodydark mb-4">
                Kategori dipakai buat mengelompokkan item peralatan (mis. <em>WO</em>, <em>Fotobooth</em>) baik di template maupun di tiap project.
              </p>

              <!-- Form tambah kategori -->
              <form action="<?= site_url('peralatan-event/kategori/store') ?>" method="post" class="flex flex-col sm:flex-row gap-2 mb-6">
                <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">
                <input type="text" name="nama_kategori" required placeholder="Nama kategori, misalnya: WO"
                ...
                <div x-data="{ editing: false }" class="rounded-md border border-stroke dark:border-strokedark p-4">
                  <div x-show="!editing" class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <div class="flex flex-wrap gap-2">
                      <a href="<?= site_url('peralatan-event/kategori/move-up/' . $kat->id_session) ?>" class="px-3 py-1.5 border rounded-md text-sm <?= $i === 0 ? 'opacity-40 pointer-events-none' : 'hover:bg-whiter dark:hover:bg-meta-4' ?>">&uarr;</a>
                      <a href="<?= site_url('peralatan-event/kategori/move-down/' . $kat->id_session) ?>" class="px-3 py-1.5 border rounded-md text-sm <?= $i === $total - 1 ? 'opacity-40 pointer-events-none' : 'hover:bg-whiter dark:hover:bg-meta-4' ?>">&darr;</a>
                      ...
                      <a href="<?= site_url('peralatan-event/kategori/delete/' . $kat->id_session) ?>"
                        onclick="return confirm('Hapus kategori ini beserta SEMUA item di dalamnya (termasuk yang sudah disalin ke project)?')"
                        class="px-3 py-1.5 border border-red-300 text-red-600 rounded-md text-sm">Hapus</a>
                    </div>
                  </div>
                  <form x-show="editing" action="<?= site_url('peralatan-event/kategori/update/' . $kat->id_session) ?>" method="post" class="flex flex-col sm:flex-row gap-2">
                    <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">
