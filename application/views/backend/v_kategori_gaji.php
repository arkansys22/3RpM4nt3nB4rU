<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Kategori Gaji</title>
    <link rel="icon" href="<?php echo base_url()?>assets/backend/mb.png" type="image/x-icon">
    <link href="<?php echo base_url()?>assets/backend/style.css" rel="stylesheet" type="text/css"/>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body
    x-data="{ page: 'dashboard', 'loaded': true, 'darkMode': true, 'stickyMenu': false, 'sidebarToggle': false, 'scrollTop': false }"
    x-init="
         darkMode = JSON.parse(localStorage.getItem('darkMode'));
         $watch('darkMode', value => localStorage.setItem('darkMode', JSON.stringify(value)))"
    :class="{'dark text-bodydark bg-boxdark-2': darkMode === true}"
  >
  <!-- ===== Preloader Start ===== -->
  <div x-show="loaded" x-init="window.addEventListener('DOMContentLoaded', () => {setTimeout(() => loaded = false, 500)})" class="fixed left-0 top-0 z-999999 flex h-screen w-screen items-center justify-center bg-white dark:bg-black">
    <div class="h-16 w-16 animate-spin rounded-full border-4 border-solid border-primary border-t-transparent">
    </div>
  </div>
  <!-- ===== Preloader End ===== -->
  <!-- ===== Page Wrapper Start ===== -->
  <div class="flex h-screen overflow-hidden">
    <?php $this->load->view('backend/sidebar')?>

    <!-- ===== Content Area Start ===== -->
    <div class="relative flex flex-1 flex-col overflow-y-auto overflow-x-hidden">
      <?php $this->load->view('backend/header')?>

      <!-- ===== Main Content Start ===== -->
      <main>
        <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
          <div class="grid grid-cols-12 gap-4 md:gap-6 2xl:gap-9">
            <div class="col-span-12 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
                <div class="flex justify-between items-center mb-4">
                <h1 class="text-2xl font-bold">Kategori Gaji</h1>
                <a href="<?= site_url('rekap-gaji') ?>" class="flex items-center gap-2 bg-blue-500 text-white p-3 rounded-md hover:bg-blue-700 focus:outline-none">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 15 3 9m0 0 6-6M3 9h12a6 6 0 0 1 0 12h-3" />
                  </svg>
                </a>
                </div>

                <?php if ($this->session->flashdata('error')): ?>
                  <div class="mb-4 p-3 rounded-md bg-red-100 text-red-700 text-sm">
                    <?= $this->session->flashdata('error') ?>
                  </div>
                <?php endif; ?>
                <?php if ($this->session->flashdata('success')): ?>
                  <div class="mb-4 p-3 rounded-md bg-green-100 text-green-700 text-sm">
                    <?= $this->session->flashdata('success') ?>
                  </div>
                <?php endif; ?>

                <?php $sedang_edit = !empty($kategori); ?>

                <div class="mb-6">
                  <button type="button" id="btn-toggle-form-kategori" onclick="toggleFormKategoriGaji()" class="bg-green-500 text-white px-5 py-2.5 rounded-md hover:bg-green-600 font-semibold <?= $sedang_edit ? 'hidden' : '' ?>">
                    + Tambah Kategori
                  </button>

                  <div id="form-kategori-wrapper" class="<?= $sedang_edit ? '' : 'hidden' ?> mt-4 p-5 rounded-lg bg-gray-50 dark:bg-neutral-700">
                    <h2 class="font-semibold text-gray-800 dark:text-white mb-3">
                      <?= $sedang_edit ? 'Edit Kategori' : 'Tambah Kategori Baru' ?>
                    </h2>
                    <form method="post" action="<?= $sedang_edit ? site_url('rekap-gaji/kategori/update/' . $kategori->id) : site_url('rekap-gaji/kategori/store') ?>">
                      <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">
                      <div class="mb-4">
                        <label class="mb-2 block text-sm text-gray-700 dark:text-gray-200">Nama Kategori</label>
                        <input
                          type="text"
                          name="nama_kategori"
                          required
                          maxlength="100"
                          value="<?= $sedang_edit ? $kategori->nama_kategori : '' ?>"
                          placeholder="Contoh: Staff Sales Senior"
                          class="w-full sm:w-96 rounded-md border border-stroke dark:border-strokedark px-3 py-2 dark:bg-boxdark dark:text-white"
                        >
                      </div>
                      <div class="mb-4">
                        <label class="mb-2 block text-sm text-gray-700 dark:text-gray-200">Nominal Gaji</label>
                        <input
                          type="number"
                          name="nominal_gaji"
                          required
                          min="0"
                          step="1000"
                          value="<?= $sedang_edit ? $kategori->nominal_gaji : '' ?>"
                          placeholder="Contoh: 3000000"
                          class="w-full sm:w-60 rounded-md border border-stroke dark:border-strokedark px-3 py-2 dark:bg-boxdark dark:text-white"
                        >
                      </div>
                      <div class="flex gap-3">
                        <button type="submit" class="bg-green-500 text-white px-5 py-2.5 rounded-md hover:bg-green-600 font-semibold">
                          Simpan
                        </button>
                        <?php if ($sedang_edit): ?>
                          <a href="<?= site_url('rekap-gaji/kategori') ?>" class="bg-gray-400 text-white px-5 py-2.5 rounded-md hover:bg-gray-500 font-semibold">
                            Batal
                          </a>
                        <?php else: ?>
                          <button type="button" onclick="toggleFormKategoriGaji()" class="bg-gray-400 text-white px-5 py-2.5 rounded-md hover:bg-gray-500 font-semibold">
                            Batal
                          </button>
                        <?php endif; ?>
                      </div>
                    </form>
                  </div>
                </div>

                <div class="overflow-x-auto bg-white dark:bg-neutral-700 rounded-lg shadow-md">
                  <table class="min-w-full text-left text-sm whitespace-nowrap bg-white dark:bg-neutral-800">
                    <thead class="uppercase tracking-wider border-b-2 border-gray-200 dark:border-neutral-600 bg-gray-100 dark:bg-neutral-700">
                      <tr>
                        <th scope="col" class="px-6 py-4 text-gray-700 dark:text-gray-300">Nama Kategori</th>
                        <th scope="col" class="px-6 py-4 text-gray-700 dark:text-gray-300">Nominal Gaji</th>
                        <th scope="col" class="px-6 py-4 text-gray-700 dark:text-gray-300">Aksi</th>
                      </tr>
                    </thead>
                    <tbody>
                      <?php if (empty($daftar_kategori)): ?>
                        <tr class="border-b border-gray-200 dark:border-neutral-600">
                          <td colspan="3" class="px-6 py-4 text-gray-700 dark:text-gray-400 text-center">
                            Belum ada kategori gaji.
                          </td>
                        </tr>
                      <?php else: ?>
                        <?php foreach ($daftar_kategori as $k): ?>
                          <tr class="border-b border-gray-200 dark:border-neutral-600">
                            <th scope="row" class="px-6 py-4 text-gray-900 dark:text-gray-200">
                              <?= $k->nama_kategori ?>
                            </th>
                            <td class="px-6 py-4 text-gray-700 dark:text-gray-400">
                              Rp <?= number_format($k->nominal_gaji, 0, ',', '.') ?>
                            </td>
                            <td class="px-6 py-4">
                              <a href="<?= site_url('rekap-gaji/kategori/edit/' . $k->id) ?>" class="bg-blue-500 text-white px-3 py-1 rounded-md hover:bg-blue-600">
                                Edit
                              </a>
                              <a
                                href="<?= site_url('rekap-gaji/kategori/delete/' . $k->id) ?>"
                                onclick="return confirm('Hapus kategori &quot;<?= addslashes($k->nama_kategori) ?>&quot;? User yang masih pakai kategori ini akan jadi belum-berkategori.')"
                                class="bg-red-500 text-white px-3 py-1 rounded-md hover:bg-red-600"
                              >
                                Hapus
                              </a>
                            </td>
                          </tr>
                        <?php endforeach; ?>
                      <?php endif; ?>
                    </tbody>
                  </table>
                </div>
            </div>
          </div>
        </div>
      </main>
      <!-- ===== Main Content End ===== -->
    </div>
    <!-- ===== Content Area End ===== -->
  </div>
  <script defer src="<?php echo base_url()?>assets/backend/bundle.js"></script>
  <script>
    function toggleFormKategoriGaji() {
      var wrapper = document.getElementById('form-kategori-wrapper');
      var btn = document.getElementById('btn-toggle-form-kategori');
      wrapper.classList.toggle('hidden');
      btn.classList.toggle('hidden');
    }
  </script>
</body>
</html>
