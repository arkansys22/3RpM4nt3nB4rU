...
foreach ($kegiatan_list as $item) {
    $slides[] = [
        ...
        'foto'          => !empty($item->foto) ? base_url('assets/uploads/susunan_acara/' . $item->foto) : null,
    ];
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Presentasi — <?= htmlspecialchars($kategori->nama_kategori) ?></title>
...
<body class="h-screen w-screen overflow-hidden text-white"
  x-data="{
    kategori: <?= htmlspecialchars(json_encode($kategori->nama_kategori), ENT_QUOTES, 'UTF-8') ?>,
    slides: <?= htmlspecialchars(json_encode($slides), ENT_QUOTES, 'UTF-8') ?>,
    i: -1, /* -1 = slide judul kategori */
  ...
  x-init="
    window.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Escape') window.location.href = '<?= site_url('susunan-acara/lihat/' . $kategori->id_session) ?>';
    });
  "
>

  <!-- Tombol keluar -->
  <a href="<?= site_url('susunan-acara/lihat/' . $kategori->id_session) ?>"
  ...
  <div class="relative h-full w-full flex items-center justify-center px-6 sm:px-16">
    ...
    <!-- Slide 0: judul kategori -->
    <div x-show="i === -1" x-transition.opacity.duration.500ms class="text-center">
      <p class="uppercase tracking-[0.3em] text-primary text-sm sm:text-base mb-4">Susunan Acara</p>
    ...
    <template x-for="(s, idx) in slides" :key="idx">
      <div x-show="i === idx" x-transition.opacity.duration.500ms
        ...
        <div class="order-2 md:order-1">
          <p class="text-primary font-semibold text-sm sm:text-base mb-2" x-text="'Kegiatan ' + (idx + 1) + ' dari ' + slides.length"></p>
    ...
    <?php if (empty($slides)): ?>
    <div x-show="i > -1" class="text-center text-white/50">Belum ada kegiatan di kategori ini.</div>
    <?php endif; ?>
