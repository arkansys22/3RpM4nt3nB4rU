...
<head>
    ...
    <title>List Peralatan Event — <?= htmlspecialchars($project->project_name ?? '') ?></title>
          ...
          <div class="grid grid-cols-12 gap-4 md:gap-6 2xl:gap-9">
            <div class="col-span-12 lg:col-span-10 lg:col-start-2 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
              ...
              <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-2">
                <h1 class="text-2xl font-bold">List Peralatan Event</h1>
                <div class="flex flex-wrap gap-2">
                  <?php if ($ada_item && $kategori_aktif): ?>
                  <a href="<?= site_url('peralatan-event-project/cetak/' . $project->id_session . '/' . $kategori_aktif) ?>" target="_blank" class="px-4 py-2 border rounded-md font-medium hover:bg-whiter dark:hover:bg-meta-4">
                    Cetak
                  </a>
                  <?php endif; ?>
              ...
              <p class="text-sm text-body dark:text-bodydark mb-4"><?= htmlspecialchars($project->project_name ?? '') ?></p>

              <!-- Filter tampilan per kategori -->
              <?php if (count($kategori_list) > 1): ?>
              <div class="flex flex-wrap gap-2 mb-4">
                <?php foreach ($kategori_list as $kat): ?>
                <a href="<?= site_url('peralatan-event-project/' . $project->id_session . '/' . $kat->id_session) ?>"
                  class="px-3 py-1.5 rounded-md text-sm border <?= $kategori_aktif === $kat->id_session ? 'bg-primary text-white border-primary' : 'hover:bg-whiter dark:hover:bg-meta-4' ?>"><?= htmlspecialchars($kat->nama_kategori) ?></a>
                <?php endforeach; ?>
              </div>
              <?php endif; ?>
              ...
              <?php if (!$ada_item && $kategori_aktif): ?>
              <div class="mb-6 p-4 rounded-md border border-dashed border-stroke dark:border-strokedark">
                <p class="text-sm text-body dark:text-bodydark mb-3">Belum ada item di kategori ini. Mulai dengan menyalin dari template, lalu sesuaikan per project ini.</p>
                <a href="<?= site_url('peralatan-event-project/salin/' . $project->id_session . '/' . $kategori_aktif) ?>" class="px-4 py-2 bg-primary text-white rounded-md font-medium hover:bg-opacity-90 inline-block">
                  Salin dari Template
                </a>
              </div>
              <?php endif; ?>
              ...
              <?php if (empty($grouped)): ?>
                <p class="text-sm text-body dark:text-bodydark border border-dashed border-stroke dark:border-strokedark rounded-md p-4">
                  Belum ada kategori peralatan. <a href="<?= site_url('peralatan-event/kategori') ?>" class="text-primary hover:underline">Buat kategori dulu</a> di Template Peralatan Event.
                </p>
              ...
              <div class="mb-8">
                  ...
                  <div x-data="{ editing: false }" class="rounded-md border border-stroke dark:border-strokedark p-3">
                    <div x-show="!editing" class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                      <span class="<?= $item->bold ? 'font-bold' : '' ?> text-black dark:text-white">
                        <?= $i + 1 ?>. <?= htmlspecialchars($item->nama_barang) ?>
                        <?php if ($item->detail !== ''): ?><span class="font-normal text-body dark:text-bodydark"> — <?= htmlspecialchars($item->detail) ?></span><?php endif; ?>
                      </span>
                      <div class="flex flex-wrap gap-2">
                        <a href="<?= site_url('peralatan-event-project/move-up/' . $item->id_session) ?>" class="px-3 py-1.5 border rounded-md text-sm <?= $i === 0 ? 'opacity-40 pointer-events-none' : 'hover:bg-whiter dark:hover:bg-meta-4' ?>">&uarr;</a>
                        <a href="<?= site_url('peralatan-event-project/move-down/' . $item->id_session) ?>" class="px-3 py-1.5 border rounded-md text-sm <?= $i === $total - 1 ? 'opacity-40 pointer-events-none' : 'hover:bg-whiter dark:hover:bg-meta-4' ?>">&darr;</a>
                        ...
                        <a href="<?= site_url('peralatan-event-project/delete/' . $item->id_session) ?>"
                    ...
                    <form x-show="editing" action="<?= site_url('peralatan-event-project/update/' . $item->id_session) ?>" method="post" class="flex flex-col sm:flex-row gap-2">
                ...
                <form action="<?= site_url('peralatan-event-project/store') ?>" method="post" class="flex flex-col sm:flex-row gap-2">
                  <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">
                  ...
                  <input type="text" name="nama_barang" required placeholder="Nama barang"
                    class="flex-1 rounded-lg border border-gray-400 bg-transparent py-2 px-4 outline-none focus:border-primary dark:border-gray-600 dark:bg-form-input" />
                  <input type="text" name="detail" placeholder="Detail (mis. 5 pcs)"
                    class="flex-1 rounded-lg border border-gray-400 bg-transparent py-2 px-4 outline-none focus:border-primary dark:border-gray-600 dark:bg-form-input" />
                  <label class="flex items-center gap-2 px-2 whitespace-nowrap text-sm">
