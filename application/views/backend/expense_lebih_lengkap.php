        ...
        <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
          <div class="grid grid-cols-12 gap-4 md:gap-6 2xl:gap-9">
            <div class="col-span-12 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
                ...
                <h5 class=" font-bold">Total Expenses <span id="total_expense" class="text-sm font-medium"></span></h5>
                  ...
                  <table class="min-w-full text-left text-sm whitespace-nowrap bg-white dark:bg-neutral-800">
                    <thead class="uppercase tracking-wider border-b-2 border-gray-200 dark:border-neutral-600 bg-gray-100 dark:bg-neutral-700">
                      <tr>
                        ...
                        <th scope="col" class="px-6 py-4 text-gray-700 dark:text-gray-300">Expenses</th>
                      </tr>
                    </thead>
                    <tbody>
                      <?php for ($month = 1; $month <= 12; $month++): ?>
                        <tr class="border-b border-gray-200 dark:border-neutral-600">
                          <th scope="row" class="px-6 py-4 text-gray-900 dark:text-gray-200">
                            <a href="<?= base_url("expense/lebih_lengkap/detail/$year/$month") ?>" class="text-blue-500 hover:underline">
                              <?= date('F', mktime(0, 0, 0, $month, 1)) ?>
                            </a>
                          </th>
                          <td class="px-6 py-4 text-gray-700 dark:text-gray-400">
                            Rp <?= isset($months[$month]) ? number_format($months[$month]['total'], 0, ',', '.') : '0' ?>
                          </td>
    ...
    function fetchRevenueData() {
        fetch('<?= base_url('Aspanel/get_expense_data') ?>')
            ...
            .then(data => {
                ...
                document.querySelector('#total_expense').textContent = `Rp ${new Intl.NumberFormat('id-ID').format(data.total_expense)}`;
