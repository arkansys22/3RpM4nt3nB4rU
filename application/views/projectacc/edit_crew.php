...
<head>
    ...
    <title>Edit Transaksi Crew</title>
        ...
        <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
          <div class="grid grid-cols-12 gap-4 md:gap-6 2xl:gap-9">
            <div class="col-span-12 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5">
              <h1 class="text-2xl font-bold mb-4">Edit Crew <?= $project->project_name ?> <?= $project->event_date ?> | <?= $project->location ?></h1>
              <form action="<?= site_url('finance-project/update2/' . $project->id_session . '/' . $transaction->id_session) ?>" method="post" class="bg-white p-6 shadow-md rounded">

                <input type="hidden" name="project_id_session" value="<?= $project->id_session ?>">

                <label class="block mb-2">Nama Transaksi Crew</label>
                <input type="text" name="nama_transaksi" class="w-full px-4 py-2 border rounded mb-4" value="<?= $transaction->nama_transaksi ?>" required>

                <label class="block mb-2">Pilih Crew</label>
                <select name="detail" class="w-full px-4 py-2 border rounded mb-4" required> 
                        <option value="-">-</option>
                        <?php foreach ($crews as $p): ?>
                            <option value="<?= $p['id_session'] ?>" <?= $transaction->detail == $p['id_session'] ? 'selected' : '' ?>>
                                <?= $p['role'] ?> - <?= $p['crew_name'] ?>
                            </option>
                        <?php endforeach; ?>
                  </select>
                ...
                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded w-full hover:bg-blue-600 sm:w-24 mb-2 sm:mb-0 text-center">Simpan</button>
